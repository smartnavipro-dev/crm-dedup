...
with st.sidebar:
    ...
    with st.expander("📖 使い方"):
        st.markdown("""
**① CSVをアップロード**
CRMからエクスポートしたCSVを選択します。UTF-8・Shift-JIS対応、最大5,000行。

**② ルールを設定**
- **チェック項目**: 重複を調べるメイン列を選択（会社名・氏名など）
- **スライダー**: 78%推奨。上げると厳格、下げると広く検出
- **追加ルール**: メールアドレスや電話番号を加えると精度が上がります

**③ 解析**
自動で実行されます。しばらくお待ちください。

**④ レビュー**
検出されたグループを1件ずつ確認します。
- ✅ **選択を残して他を削除** — 代表1件を残し残りを削除候補に
- ❌ **別人・別会社** — 誤検知として全件残す
- ⏭ **スキップ** — 判断を保留（全件残る）

**⑤ エクスポート**
- `cleaned.csv` — 重複除去済みデータ
- `deleted_records.csv` — 削除対象データ（CRMでの照合用）
""")
    st.divider()
    st.caption(
        "**プライバシー**\n\n"
...
st.markdown("CRMの重複データを自動で検出・整理します。CSVをアップロードするだけ、無料で使えます。")
